...
## Free HTTPS without buying a domain (recommended)

The self-signed cert generated by `nginx/generate-self-signed-cert.sh`
works, but a browser frontend on a *different* origin (e.g. the Next.js
app in `shop-ops-ai-frontend/`, hosted on Vercel) will hard-refuse
`fetch()`/XHR calls to it — self-signed certs aren't trusted cross-origin.
Getting a real trusted cert doesn't require buying a domain:
[nip.io](https://nip.io) resolves `<ip-with-dashes>.nip.io` to that IP for
free, which is enough for Let's Encrypt to issue a real cert against it.

```bash
# on the EC2 box, from /opt/shopops, with the stack already up:
./nginx/generate-letsencrypt-cert.sh <elastic-ip> <your-email>
docker compose -f docker-compose.prod.yml up -d
```

Then update `CORS_ORIGINS` in `.env.prod` to the frontend's real origin
(unchanged), and point the frontend's `NEXT_PUBLIC_API_BASE_URL` (e.g. a
Vercel env var) at `https://<ip-with-dashes>.nip.io` instead of the raw
IP. After this, nobody needs to click through a certificate warning —
the cert is genuinely trusted. Let's Encrypt certs expire in 90 days;
re-run the same script to renew (safe to re-run).

If you'd rather not bother with this yet, the self-signed cert still
works — see the workaround below.

## Known limitation: browser CORS + self-signed cert (if not using the free fix above)

Until the nip.io + Let's Encrypt swap above is done, there are two
workarounds for the self-signed cert's hard browser refusal:
...
2. Swap in Let's Encrypt as described above whenever you're ready — same
   Nginx config structure, just a different cert source.
